...
async def migrate() -> None:
    logging.info("[Migrate] Connecting to database...")
    ...
    try:
        ...
        logging.error(f"[Migrate] Migration transaction failed: {e}")
        raise e
    finally:
        await conn.close()

async def seed() -> None:
    logging.info("[Seed] Connecting to database...")
    conn = await asyncpg.connect(env.DATABASE_URL)
    
    try:
        async with conn.transaction():
            # Check if game packages already exist
            count = await conn.fetchval("SELECT COUNT(*) FROM game_packages")
            if count > 0:
