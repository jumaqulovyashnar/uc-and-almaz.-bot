...
export async function generateClickCheckoutLink(orderId: string, returnUrl?: string) {
  const res = await fetch(`${API_BASE}/click/checkout-link`, {
    method: 'POST',
    headers: getHeaders(),
...
export async function createClickInvoice(orderId: string, phoneNumber: string) {
  const res = await fetch(`${API_BASE}/click/invoice/create`, {
    method: 'POST',
    headers: getHeaders(),
...
export async function checkClickInvoice(invoiceId: string, merchantTransId?: string) {
  const query = merchantTransId ? `?merchant_trans_id=${merchantTransId}` : '';
  const res = await fetch(`${API_BASE}/click/invoice/check/${invoiceId}${query}`, {
    headers: getHeaders(),
  });
  return safeJson(res);
}

export async function createClickCardToken(cardNumber: string, expireDate: string, phoneNumber: string) {
  const res = await fetch(`${API_BASE}/click/token/create`, {
    method: 'POST',
    headers: getHeaders(),
    body: JSON.stringify({ cardNumber, expireDate, phoneNumber })
  });
  return safeJson(res);
}

export async function verifyClickCardToken(cardToken: string, smsCode: string) {
  const res = await fetch(`${API_BASE}/click/token/verify`, {
    method: 'POST',
    headers: getHeaders(),
...
export async function payWithClickCardToken(orderId: string, cardToken: string) {
  const res = await fetch(`${API_BASE}/click/token/pay`, {
    method: 'POST',
    headers: getHeaders(),
...
export async function deleteClickCardToken(cardToken: string) {
  const res = await fetch(`${API_BASE}/click/token/${cardToken}`, {
    method: 'DELETE',
    headers: getHeaders(),
  });
  return safeJson(res);
